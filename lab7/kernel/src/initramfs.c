...
int register_initramfs()
{
    ...
    fs.name = "initramfs";
    fs.setup_mount = initramfs_setup_mount;//set initramfs.setup_mount to func(initramfs_setup_mount) for later use
...
int initramfs_setup_mount(struct filesystem *fs, struct mount *_mount)
{
    ...
    while (header_pointer != 0)
    {
        ...
        if (header_pointer != 0)
        {
            ...
            fileinode->datasize = filesize; // file_t
            fileinode->name = filepath; // 4KB
